...
SCANS = {(Directions.N, Directions.NE, Directions.NW): Directions.N, (Directions.S, Directions.SE, Directions.SW): Directions.S, \
    (Directions.W, Directions.NW, Directions.SW): Directions.W, (Directions.E, Directions.NE, Directions.SE): Directions.E}
...
def get_direction(elf, elves):
    ...
    new_direction = potential_directions[0] if 0 < len(potential_directions) < 4 else Directions.NIL
...
def box_elves(elves):
    ...
    return (abs(all_i[0]) + abs(all_i[-1]) + 1) * (abs(all_j[0]) + abs(all_j[-1]) + 1) - len(elves)
